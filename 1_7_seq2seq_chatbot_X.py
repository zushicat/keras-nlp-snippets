...
from _load_data import load_data_with_intent_from_yml, tag_answers
...
def get_data():
    '''
    Categories (defined in yml files like _something_) are added to start of question and answer strings
    '''
    questions, answers = load_data_with_intent_from_yml("../../data/chatbot_data/karin", "conversations")
...
def build_model(vocab_size, hidden_layer, max_len_sequence, embedding_matrix):
    ...
    _, state_h, state_c = LSTM(hidden_layer, return_sequences=True, return_state=True, dropout=0.5, name="enc_lstm")(encoder_embedding)
    ...
    decoder_lstm = LSTM(hidden_layer, return_sequences=True, return_state=True, dropout=0.5, name="dec_lstm")
...
def build_inference_encoder_decoder(model, hidden_layer):
    ...
    _, state_h, state_c = model.get_layer("enc_lstm").output   # lstm
    encoder_states = [state_h, state_c]
    ...
    decoder_lstm = model.get_layer("dec_lstm")
...
def save_lstm_model(tokenizer, model, model_name, max_len_sequence_out):
    ...
    with open(f"models/1_7_seq2seq_chatbot/{model_name}_config.json", "w") as f:
        f.write(model_config)
    model.save_weights(f"models/1_7_seq2seq_chatbot/{model_name}_weights.h5")  # serialize weights to HDF5

    with open(f"models/1_7_seq2seq_chatbot/{model_name}_tokenizer.pickle", "wb") as f:
        pickle.dump(tokenizer, f, protocol=pickle.HIGHEST_PROTOCOL)

def load_lstm_model(model_name):
    with open(f"models/1_7_seq2seq_chatbot/{model_name}_config.json") as f:
        model_config = f.read()
    model = model_from_json(model_config)
    model.load_weights(f"models/1_7_seq2seq_chatbot/{model_name}_weights.h5")

    with open(f"models/1_7_seq2seq_chatbot/{model_name}_tokenizer.pickle", "rb") as f:
...
def predict_sequence(
    ...
    for _ in range(max_len_sequence_out):
        decoder_output, h, c = inference_decoder.predict([decoder_input] + state)
...
def predict(model_name):
    ...
    # predict sequence
    # ***********
    questions_test = ["do you like dogs?", "do you know nltk"]
    answers_test = ["yes", "yes"]
...
# train(model_name="conversation", epochs=200)
