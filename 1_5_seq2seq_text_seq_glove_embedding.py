...
def build_model(vocab_size, hidden_layer, max_len_sequence, embedding_matrix):
    ...
    _, state_h, state_c = LSTM(hidden_layer, return_state=True, name="enc_lstm")(encoder_embedding)
    ...
    decoder_lstm = LSTM(hidden_layer, return_sequences=True, return_state=True, name="dec_lstm")
...
def build_inference_encoder_decoder(model, hidden_layer):
    ...
    decoder_outputs, state_h2, state_c2 = decoder_lstm(decoder_embedding, initial_state=decoder_states_inputs)
...
def save_lstm_model(tokenizer, model, model_name, max_len_sequence_out):
    ...
    with open(f"models/1_5_seq2seq_text_seq_glove_embedding/{model_name}_config.json", "w") as f:
        f.write(model_config)
    model.save_weights(f"models/1_5_seq2seq_text_seq_glove_embedding/{model_name}_weights.h5")  # serialize weights to HDF5

    with open(f"models/1_5_seq2seq_text_seq_glove_embedding/{model_name}_tokenizer.pickle", "wb") as f:
        pickle.dump(tokenizer, f, protocol=pickle.HIGHEST_PROTOCOL)

def load_lstm_model(model_name):
    with open(f"models/1_5_seq2seq_text_seq_glove_embedding/{model_name}_config.json") as f:
        model_config = f.read()
    model = model_from_json(model_config)
    model.load_weights(f"models/1_5_seq2seq_text_seq_glove_embedding/{model_name}_weights.h5")

    with open(f"models/1_5_seq2seq_text_seq_glove_embedding/{model_name}_tokenizer.pickle", "rb") as f:
...
def predict_sequence(
    ...
    # ***
    answer_sequence = np.zeros((1,1))
    answer_sequence[0, 0] = tokenizer.word_index.get("bos")
    ...
    for _ in range(max_len_sequence_out):
        ...
        answer_sequence = np.zeros((1,1))
        answer_sequence[0, 0] = sampled_token_index
